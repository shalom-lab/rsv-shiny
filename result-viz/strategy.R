...
model_for<-function(scenario){
  ...
  for(i in 1:repetition){
    ...
    CasesN=sum(casesN) # total RSV-ALRI hospitalization under nature condistion
    ...
    tmp_df<-data.frame(strategy=sList,seed=seeds[i],CasesN=CasesN,CasesA=CasesA,Doses=Doses,NNT=NNT)
